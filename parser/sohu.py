...
from model.models import DramaModel, DramaEpisodeModel, UrlContentModel, DramaInfoModel
import logging
    ...
    def fetch_list(self):
        ...
        for v in videos:
            ...
            try:
                ...
                pass
            if not d:
                DramaModel.instance().insert(name, v['publish_time'][:4], v['hor_w16_pic'], v['main_actor'],
                                             v['album_desc'],
                                             v['aid'], score)
                d = DramaModel.instance().get_by_name(name)
            else:
